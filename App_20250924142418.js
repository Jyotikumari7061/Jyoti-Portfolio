  ...
  return (
    <div className="App">
        ...
        <div style={{ textAlign: 'center', position: 'relative', zIndex: 10, maxWidth: '1200px', padding: '0 2rem' }}>
          ...
          <div style={{ marginBottom: '3rem' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <img
      src="/image/profile.jpg"  // ← UPDATE TO YOUR EXACT FILENAME (e.g., /image/jyoti.jpg or /image/profile.png)
      ...
      style={{
        ...
        borderRadius: '24px',
        objectFit: 'cover',
        border: '4px solid rgba(255, 255, 255, 0.8)',  // High opacity white border for visibility
        boxShadow: '0 20px 40px rgba(0, 0, 0, 0.3)',
        display: 'block !important',  // Forces display (overrides any CSS hiding)
        position: 'relative',  // Prevents overlap issues
        zIndex: 2,  // Stays on top
        backgroundColor: '#f0f0f0'  // Light bg if image fails (temporary debug)
                }}
                 onLoad={() => console.log('Image loaded successfully!')}  // Confirm load in console
      onError={(e) => {
        console.error('Local image failed (/image/profile.jpg). Check file path/filename. Trying external fallback...');
        // Step 1: Try reliable external placeholder
        e.target.src = 'https://picsum.photos/200/200?random=1';  // Random photo from Unsplash (always loads)
        // Step 2: If external fails too (rare), fallback to inline SVG
        e.target.onerror = (err) => {
          console.error('External fallback failed. Using inline SVG.');
          e.target.src = 'data:image/svg+xml;base64,';  // Blue square with custom text
          e.target.onerror = null;  // Stop loops
          e.target.style.border = '4px solid #ff0000';  // Red border on final fallback
          e.target.style.backgroundColor = '#ffeeee';  // Light red bg
        };
      }}
              />
              <div style={{
